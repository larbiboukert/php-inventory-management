	...
	if(isset($_POST['saleDetailsSaleID'])){
		...
		if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsUnitPrice) && isset($saleDetailsCustomerID)){
			...
			if(filter_var($saleDetailsQuantity, FILTER_VALIDATE_INT) === 0 || filter_var($saleDetailsQuantity, FILTER_VALIDATE_INT)){
				...
				// Quantité is not a valid number
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter a valid number for Quantité.</div>';
				exit();
			}
			...
			if(filter_var($saleDetailsUnitPrice, FILTER_VALIDATE_FLOAT) === 0.0 || filter_var($saleDetailsUnitPrice, FILTER_VALIDATE_FLOAT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter a valid number for Prix unitaire.</div>';
				exit();
			}
			...
			if($saleDetailsSaleID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter a Num Vente.</div>';
				exit();
			}
			
			// Check if customerID is empty
			if($saleDetailsCustomerID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter a Num Client.</div>';
				exit();
			}
			
			// Check if itemNumber is empty
			if($saleDetailsItemNumber == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter Num Produit.</div>';
				exit();
			}
			...
			if($saleDetailsUnitPrice == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter Prix unitaire.</div>';
				exit();
			}
			...
			if($customerIDStatement->rowCount() < 1){
				// Client id is wrong
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Num Client does not exist in DB. Please enter a valid Num Client.</div>';
				exit();
			} else {
				$row = $customerIDStatement->fetch(PDO::FETCH_ASSOC);
				$customerID = $row['customerID'];
			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				// Vente details exist in DB. Hence proceed to calculate the stock
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					// Produit numbers are different. That means the user wants to update a new item number too
					...
					if($newItemCurrentStockStatement->rowCount() < 1){
						// Produit number is not in DB. Hence abort.
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Num Produit does not exist in DB. If you want to update this item, please add it to DB first.</div>';
						exit();
					}
					...
					// Get the Stock actuel of the previous item
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Vente details updated.</div>';
					exit();
					
				} else {
					// Produit numbers are equal. That means item number is valid
					...
					if($stockStatement->rowCount() > 0){
						// Produit exists in the item table, therefore, start updating data in sale table
						...
						// Mettre à jour the new stock value in item table.
						...
						// Produit does not exist in item table, therefore, you can't update 
						// sale details for it 
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Produit does not exist in DB. Therefore, first enter this item to DB using the <strong>Produit</strong> tab.</div>';
						exit();
					}	
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Vente details does not exist in DB for the given Num Vente. Therefore, can\'t update.</div>';
				exit();
