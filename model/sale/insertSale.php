	...
	if(isset($_POST['saleDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && isset($customerID) && isset($saleDate) && isset($quantity) && isset($unitPrice)){
			...
			if(filter_var($customerID, FILTER_VALIDATE_INT) === 0 || filter_var($customerID, FILTER_VALIDATE_INT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter a valid Num Client</div>';
				exit();
			}
			...
			if($stockStatement->rowCount() > 0){
				// Produit exits in DB, therefore, can proceed to a sale
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					if($customerStatement->rowCount() > 0){
						// Client exits. That means both customer, item, and stocks are available. Hence start INSERT and UPDATE
						...
						echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Vente details added to DB and stocks updated.</div>';
						exit();
						
					} else {
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Client does not exist.</div>';
						exit();
					}
				}
				
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Produit already exists in DB. Please click the <strong>Mettre à jour</strong> button to update the details. Or use a different Num Produit.</div>';
				exit();
			} else {
				// Produit does not exist, therefore, you can't make a sale from it
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Produit does not exist in DB.</div>';
				exit();
			}
