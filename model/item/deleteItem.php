	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		if(!empty($itemNumber)){
			...
			if($itemStatement->rowCount() > 0){
				...
				// Produit exists in DB. Hence start the DELETE process
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Produit deleted.</div>';
				exit();
				
			} else {
				// Produit does not exist, therefore, tell the user that he can't delete that item 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Produit does not exist in DB. Therefore, can\'t delete.</div>';
				exit();
			}
			
		} else {
			// Produit number is empty. Therefore, display the error message
