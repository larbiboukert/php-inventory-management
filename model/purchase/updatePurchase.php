	...
	if(isset($_POST['purchaseDetailsPurchaseID'])){
		...
		if(isset($purchaseDetailsItemNumber) && isset($purchaseDetailsPurchaseDate) && isset($purchaseDetailsQuantity) && isset($purchaseDetailsUnitPrice)){
			...
			if($purchaseDetailsPurchaseID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter a Num Achat.</div>';
				exit();
			}
			...
			if($originalPurchaseQuantityStatement->rowCount() > 0){
				...
				// Details Achat exist in DB. Hence proceed to calculate the stock
				...
				if($originalOrderItemNumber !== $purchaseDetailsItemNumber) {
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Details Achat added to database and stock values updated.</div>';
					exit();
					...
					if($stockStatement->rowCount() > 0){
						// Produit exists in the item table, therefore, start inserting data to purchase table
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Details Achat does not exist in DB for the given Num Achat. Therefore, can\'t update.</div>';
				exit();
