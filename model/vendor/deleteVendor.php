	...
	if(isset($_POST['vendorDetailsVendorID'])){
		...
		if(!empty($vendorDetailsVendorID)){
			...
			if($vendorStatement->rowCount() > 0){
				...
				// Fournisseur exists in DB. Hence start the DELETE process
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Fournisseur deleted.</div>';
				exit();
				
			} else {
				// Fournisseur does not exist, therefore, tell the user that he can't delete that vendor 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Fournisseur does not exist in DB. Therefore, can\'t delete.</div>';
				exit();
			}
			
		} else {
			// vendorDI is empty. Therefore, display the error message
			echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter the Fournisseur ID</div>';
			exit();
		}
	}
?>