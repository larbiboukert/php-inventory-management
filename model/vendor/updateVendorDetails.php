	...
	if(isset($_POST['vendorDetailsVendorID'])) {
		...
		if(!empty($vendorDetailsVendorID)){
			...
			if(!empty($vendorDetailsVendorFullName) && !empty($vendorDetailsVendorMobile) && !empty($vendorDetailsVendorAddress)) {
				...
				if(empty($vendorDetailsVendorID)){
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter the Fournisseur ID to update that vendor. You can find the Fournisseur ID using the Search tab</div>';
					exit();
				}
				...
				if($vendorIDSelectStatement->rowCount() > 0) {
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Fournisseur details updated.</div>';
					exit();
				} else {
					// vendorID is not in DB. Therefore, stop the update and quit
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Fournisseur ID does not exist in DB. Therefore, update not possible.</div>';
					exit();
				}
