<?php
	...
	if(isset($_POST['vendorDetailsStatus'])){
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			if($address == ''){
				// Adresse 1 is empty
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter Adresse.</div>';
				exit();
			}
			...
			echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Fournisseur added to database</div>';
		} else {
