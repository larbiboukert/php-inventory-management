	...
	if(isset($_POST['customerDetailsCustomerID'])){
		...
		if(!empty($customerDetailsCustomerID)){
			...
			if($customerStatement->rowCount() > 0){
				...
				// Client exists in DB. Hence start the DELETE process
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Client deleted.</div>';
				exit();
				
			} else {
				// Client does not exist, therefore, tell the user that he can't delete that customer 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Client does not exist in DB. Therefore, can\'t delete.</div>';
				exit();
			}
